...
return new class extends Migration
{
    ...
    {
        Schema::create('stoma_purchasemeasures', function (Blueprint $table) {
            $table->increments('purchasemeasuresid');
            $table->string('purchasemeasure', 255);
            $table->unsignedBigInteger('storeid');
            $table->dateTime('insertdate');
            $table->string('insertip', 51);
            $table->unsignedInteger('insertby')->default(0);
            $table->dateTime('editdate_pm')->nullable();
    ...
    {
        Schema::dropIfExists('stoma_purchasemeasures');
    }
};
