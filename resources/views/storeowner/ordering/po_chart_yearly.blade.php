@section('page_header', 'PO Reports Charts - Yearly')
<x-storeowner-app-layout>
    ...
    <div class="py-4">
        <div class="mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <p class="text-sm text-gray-600 mb-4">Purchases</p>
    ...
    <script>
        async function loadChart() {
            try {
                const response = await fetch('{{ route("storeowner.ordering.get_po_chart_yearly") }}', {
                    method: 'POST',
                    headers: {
                ...
                const barWidth = Math.max(30, Math.floor(barAreaWidth / jsonData.length) - 10);
                ...
                // X-axis label
                svgHTML += `<text x="${padding.left + barAreaWidth / 2}" y="${chartHeight - 10}" text-anchor="middle" font-size="12" fill="#333">Year</text>`;
                ...
                jsonData.forEach((item, index) => {
                    ...
                    svgHTML += `
                        ...
                        <rect x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" 
                              ...
                              data-year="${item.myyear}" data-value="${item.total_amount}">
                            <title>Year: ${item.myyear}, Total: €${item.total_amount.toFixed(2)}</title>
                        </rect>
                        <!-- Year label -->
                        <text x="${x + barWidth / 2}" y="${padding.top + barAreaHeight + 35}" 
                              text-anchor="middle" font-size="12" fill="#333">${item.myyear}</text>
                        <!-- Value label on top -->
                        <text x="${x + barWidth / 2}" y="${y - 5}" 
                              text-anchor="middle" font-size="11" fill="#333">${item.myyear || 'null'}</text>
                    `;
                });
