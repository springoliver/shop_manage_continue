@section('page_header', 'PO Reports Charts - Monthly')
<x-storeowner-app-layout>
    ...
    <div class="py-4">
        <div class="mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <p class="text-sm text-gray-600 mb-4">Monthly Total Orders</p>
    ...
    <script>
        async function loadChart() {
            try {
                const response = await fetch('{{ route("storeowner.ordering.get_po_chart_monthly") }}', {
                    method: 'POST',
                    headers: {
                ...
                const barWidth = Math.max(20, Math.floor(barAreaWidth / jsonData.length) - 5);
                ...
                for (let i = 0; i <= ySteps; i++) {
                    ...
                    const y = padding.top + barAreaHeight - (i / ySteps) * barAreaHeight;
                    svgHTML += `
                        <line x1="${padding.left}" y1="${y}" x2="${padding.left + barAreaWidth}" y2="${y}" stroke="#e0e0e0" stroke-width="1"/>
                        <text x="${padding.left - 15}" y="${y + 4}" text-anchor="end" font-size="11" fill="#666">${(value / maxValue).toFixed(1)}</text>
                    `;
                }

                // X-axis label
                svgHTML += `<text x="${padding.left + barAreaWidth / 2}" y="${chartHeight - 10}" text-anchor="middle" font-size="12" fill="#333">Month</text>`;
                ...
                jsonData.forEach((item, index) => {
                    ...
                    const monthLabel = item.month || 'null';
                    
                    svgHTML += `
                        <!-- Bar -->
                        <rect x="${x}" y="${y}" width="${barWidth}" height="${barHeight}" 
                              fill="#4285f4" stroke="none" class="bar-hover" 
                              data-month="${monthLabel}" data-value="${item.total_amount}">
                            <title>${monthLabel}: €${item.total_amount.toFixed(2)}</title>
                        </rect>
                        <!-- Month label (rotated) -->
                        <text x="${x + barWidth / 2}" y="${padding.top + barAreaHeight + 50}" 
                              text-anchor="middle" font-size="11" fill="#333" transform="rotate(-45 ${x + barWidth / 2} ${padding.top + barAreaHeight + 50})">${monthLabel}</text>
                        <!-- Value label on top -->
                        <text x="${x + barWidth / 2}" y="${y - 5}" 
                              text-anchor="middle" font-size="11" fill="#333">${monthLabel || 'null'}</text>
                    `;
                });
