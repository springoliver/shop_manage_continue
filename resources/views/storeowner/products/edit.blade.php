        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow p-6">
                <h2 class="text-2xl font-semibold text-gray-800 mb-6">
                    <i class="fas fa-edit mr-2"></i> Products
                </h2>
                
                <form action="{{ route('storeowner.products.update') }}" method="POST" id="myform">
                    @csrf
                    <input type="hidden" name="productid" value="{{ base64_encode($storeProduct->productid) }}">
                        ...
                        <div class="w-3/4">
                            <select name="catalog_product_groupid" id="catalog_product_groupid" 
                                ...
                                @foreach($catalogProductGroups as $group)
                                    <option value="{{ $group->catalog_product_groupid }}" {{ (old('catalog_product_groupid', $storeProduct->catalog_product_groupid) == $group->catalog_product_groupid) ? 'selected' : '' }}>
                        ...
                        <div class="w-3/4">
                            <select name="departmentid" id="departmentid" 
                                ...
                                @foreach($departments as $department)
                                    <option value="{{ $department->departmentid }}" {{ (old('departmentid', $storeProduct->departmentid) == $department->departmentid) ? 'selected' : '' }}>
                        ...
                        <div class="w-3/4">
                            <select name="supplierid" id="supplierid" 
                                ...
                                @foreach($storeSuppliers as $supplier)
                                    <option value="{{ $supplier->supplierid }}" {{ (old('supplierid', $storeProduct->supplierid) == $supplier->supplierid) ? 'selected' : '' }}>
                        ...
                        <div class="w-3/4">
                            <select name="shipmentid" id="shipmentid" 
                                ...
                                @foreach($productshipments as $shipment)
                                    <option value="{{ $shipment->shipmentid }}" {{ (old('shipmentid', $storeProduct->shipmentid) == $shipment->shipmentid) ? 'selected' : '' }}>
                                        {{ $shipment->shipment }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="w-3/4">
                            <select name="purchasepaymentmethodid" id="purchasepaymentmethodid" 
                                ...
                                @foreach($purchasePaymentMethods as $paymentMethod)
                                    <option value="{{ $paymentMethod->purchasepaymentmethodid }}" {{ (old('purchasepaymentmethodid', $storeProduct->purchasepaymentmethodid) == $paymentMethod->purchasepaymentmethodid) ? 'selected' : '' }}>
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        <label class="w-1/4 pt-2 text-sm font-medium text-gray-700 text-end pr-5">
                            Tax Bands <span class="text-red-500">*</span>
                        </label>
                        <div class="w-3/4">
                            <select name="taxid" id="taxid" 
                                ...
                                @foreach($taxSettings as $tax)
                                    <option value="{{ $tax->taxid }}" {{ (old('taxid', $storeProduct->taxid) == $tax->taxid) ? 'selected' : '' }}>
                                        {{ $tax->tax_name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        <label class="w-1/4 pt-2 text-sm font-medium text-gray-700 text-end pr-5">
                            Purchasing Measures <span class="text-red-500">*</span>
                        </label>
                        <div class="w-3/4">
                            <select name="purchasemeasuresid" id="purchasemeasuresid" 
                                    class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500" required>
                                <option value="">Select Purchasing Measures</option>
                                @foreach($purchaseMeasures as $measure)
                                    <option value="{{ $measure->purchasemeasuresid }}" {{ (old('purchasemeasuresid', $storeProduct->purchasemeasuresid) == $measure->purchasemeasuresid) ? 'selected' : '' }}>
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="w-3/4">
                            <input type="text" name="product_name" id="product_name" 
                                   value="{{ old('product_name', $storeProduct->product_name) }}"
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="w-3/4">
                            <input type="text" name="product_price" id="product_price" 
                                   value="{{ old('product_price', $storeProduct->product_price) }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500" 
                                   required>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="w-3/4">
                            <input type="text" value="{{ $storeProduct->editdate ? \Carbon\Carbon::parse($storeProduct->editdate)->format('Y-m-d') : '-' }}" 
                                   disabled
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="w-3/4">
                            <input type="text" name="product_notes" id="product_notes" 
                                   value="{{ old('product_notes', $storeProduct->product_notes) }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500">
                        </div>
                    </div>
