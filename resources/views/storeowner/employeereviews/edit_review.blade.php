    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow p-6">
                <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Review</h2>
                
                <form action="{{ route('storeowner.employeereviews.update-review') }}" method="POST" id="myform">
                    @csrf
                    ...
                    @if($reviews->count() > 0)
                                    ...
                                    @foreach($reviews as $review)
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                ...
                                                <input type="hidden" name="emp_reviewid[]" value="{{ $review->emp_reviewid }}" />
                                            </td>
                                            <td class="px-6 py-4">
                                                <textarea name="comments[]" rows="3" 
                                                          class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500">{{ $review->comments }}</textarea>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="date" name="next_review_date" id="next_review_date" 
                                   value="{{ $reviews->first()->next_review_date ? \Carbon\Carbon::parse($reviews->first()->next_review_date)->format('Y-m-d') : '' }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500" 
                                   required>
                            <p class="mt-2 text-sm text-gray-500">This date will be applied to all subjects in this review.</p>
                        </div>
                    @endif
