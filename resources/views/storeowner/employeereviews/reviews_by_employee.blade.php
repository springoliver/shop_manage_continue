<x-storeowner-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($reviews->count() > 0)
                <div class="mb-6">
                    <a href="{{ route('storeowner.employeereviews.add-review', base64_encode($reviews->first()->employeeid)) }}" 
                       class="px-4 py-2 bg-gray-800 text-white rounded-md hover:bg-gray-700">
                        <i class="fas fa-plus mr-2"></i> Add Review for {{ ucfirst($reviews->first()->firstname ?? '') }} {{ ucfirst($reviews->first()->lastname ?? '') }}
                    </a>
                </div>
            @endif
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-800">Reviews for</h3>
                </div>
                                ...
                                @foreach($reviews as $review)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ $review->insertdatetime ? \Carbon\Carbon::parse($review->insertdatetime)->format('Y-m-d H:i:s') : '-' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <a href="#" 
                                               ...
                                               class="text-red-600 hover:text-red-800 mr-3" title="Delete">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                            <a href="{{ route('storeowner.employeereviews.view', base64_encode($review->emp_reviewid)) }}" 
