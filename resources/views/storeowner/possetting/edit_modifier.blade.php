        ...
        <nav class="flex text-gray-500 text-sm" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <i class="fas fa-chevron-right text-gray-400"></i>
                        <a href="{{ route('storeowner.possetting.modifiers') }}" class="ml-1 hover:text-gray-700">Modifiers</a>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <h2 class="text-2xl font-semibold text-gray-800 mb-6">
                    <i class="fas fa-align-justify mr-2"></i> Edit Order Modifier
                </h2>
                
                <form action="{{ route('storeowner.possetting.update-modifier') }}" method="POST" id="myform">
                    @csrf
                    <input type="hidden" name="pos_modifiers_id" value="{{ base64_encode($modifier->pos_modifiers_id) }}">
                    ...
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="w-3/4">
                            <input type="text" name="pos_modifier_name" id="pos_modifier_name" 
                                   value="{{ old('pos_modifier_name', $modifier->pos_modifier_name) }}"
                            ...
                            <select name="catalog_product_groupid" id="catalog_product_groupid" 
                                ...
                                @foreach($productGroups as $productGroup)
                                    <option value="{{ $productGroup->catalog_product_groupid }}" 
                                            {{ ($modifier->catalog_product_groupid == $productGroup->catalog_product_groupid || old('catalog_product_groupid') == $productGroup->catalog_product_groupid) ? 'selected' : '' }}>
