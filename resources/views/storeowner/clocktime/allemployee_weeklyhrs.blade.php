<x-storeowner-app-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 font-medium text-gray-700 md:ml-2">Weekly Hours</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow mb-6">
                <ul class="nav nav-tabs flex border-b border-gray-200" role="tablist">
                    <li class="nav-item">
                        <a href="{{ route('storeowner.clocktime.reports-chart-weekly') }}" 
                           class="px-4 py-2 block text-gray-600 hover:text-gray-800 hover:border-gray-300 border-b-2 border-transparent">
                            Weekly Chart View
                        </a>
                    </li>
                ...
                <div class="p-6">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200" id="table-new">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200" id="weeklyHoursTableBody">
                                @if($empPayrollHrs->count() > 0)
                                    @foreach($empPayrollHrs as $payroll)
                                        ...
                                        @endphp
                                        <tr class="weekly-hours-row hover:bg-gray-50" 
                                            data-row-index="{{ $loop->index }}"
                                            data-week="{{ strtolower($payroll->weekno . ' ' . $weekStartDate) }}"
                                            data-month="{{ strtolower($monthName) }}"
                                            data-year="{{ $payroll->year }}"
                                            data-hours="{{ (float)$payroll->hours_worked }}">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ $payroll->weekno }} - {{ $weekStartDate }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {{ $monthName }}
                                            </td>
    ...
    <script>
        ...
        function initializePagination() {
            const tbody = document.getElementById('weeklyHoursTableBody');
            allRows = Array.from(tbody.querySelectorAll('tr.weekly-hours-row'));
            filteredRows = [...allRows];
            
            const noWeeklyHoursRow = document.getElementById('noWeeklyHoursRow');
            if (noWeeklyHoursRow && allRows.length > 0) {
                noWeeklyHoursRow.style.display = 'none';
            }
        ...
        function updateDisplay() {
            ...
            if (sortColumn && filteredRows.length > 0) {
                ...
                filteredRows.forEach(row => {
                    if (row.id !== 'noWeeklyHoursRow') {
                        if (noWeeklyHoursRow && noWeeklyHoursRow.parentNode) {
                            tbody.insertBefore(row, noWeeklyHoursRow);
                        } else {
            ...
            if (noWeeklyHoursRow) {
                if (filteredRows.length === 0) {
                    noWeeklyHoursRow.style.display = '';
                } else {
                    noWeeklyHoursRow.style.display = 'none';
                }
            }
            ...
            for (let i = start; i < end; i++) {
                if (filteredRows[i] && filteredRows[i].id !== 'noWeeklyHoursRow') {
