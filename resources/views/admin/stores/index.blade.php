...
<x-admin-app-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 font-medium text-gray-700 md:ml-2">Stores</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.stores.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white text-sm font-medium rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($stores as $store)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $store->getStoreTypeName() }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button type="button" onclick="openStatusModal({{ $store->storeid }}, '{{ $store->status }}')" 
                                        class="px-2 py-1 text-xs font-semibold rounded-full {{ $store->status === 'Active' ? 'bg-green-100 text-green-800' : ($store->status === 'Pending Setup' ? 'bg-yellow-100 text-yellow-800' : ($store->status === 'Suspended' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800')) }}">
                                        {{ $store->status }}
                                    </button>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-3">
                                        <!-- View -->
                                        <a href="{{ route('admin.stores.show', $store) }}" class="text-blue-600 hover:text-blue-900" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <!-- Edit -->
                                        <a href="{{ route('admin.stores.edit', $store) }}" class="text-gray-600 hover:text-gray-900" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <!-- Delete -->
                                        <form action="{{ route('admin.stores.destroy', $store) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this store?');">
                                            @csrf
                                            @method('DELETE')
    ...
    <script>
        function openStatusModal(storeid, currentStatus) {
            ...
            form.action = '{{ route("admin.stores.change-status", ":id") }}'.replace(':id', storeid);
