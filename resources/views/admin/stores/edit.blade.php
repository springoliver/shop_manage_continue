...
<x-admin-app-layout>
                ...
                <div class="grid grid-cols-1 gap-6">
                        ...
                        <select name="storeownerid" id="storeownerid"
                            ...
                            @foreach($storeOwners as $owner)
                                <option value="{{ $owner->ownerid }}"
                                    {{ (old('storeownerid', $store->storeownerid) == $owner->ownerid) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="typeid" id="typeid"
                            ...
                            @foreach($storeTypes as $type)
                                <option value="{{ $type->typeid }}" {{ old('typeid', $store->typeid ?? '') == $type->typeid ? 'selected' : '' }}>
                                    {{ $type->store_type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="email" name="manager_email" id="manager_email"
                            value="{{ old('manager_email', $store->manager_email) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-gray-500 focus:border-gray-500">
                        @error('manager_email')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="full_google_address" id="full_google_address"
                            value="{{ old('full_google_address', $store->full_google_address) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-gray-500 focus:border-gray-500"
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex-shrink-0">
                                @if($store->logofile)
                                    <img id="logo-preview" src="{{ asset('storage/' . $store->logofile) }}" alt="Logo Preview" class="w-32 h-32 object-cover border-2 border-gray-300 rounded">
                                @else
                            ...
                            <div class="flex-1">
                                <div class="mb-2">
                                    <p class="text-sm text-gray-600">Do you want to update image?</p>
                                    <div class="flex items-center space-x-4 mt-2">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="update_logo" value="yes" class="form-radio" onchange="toggleLogoUpload(true)">
                                            <span class="ml-2">Yes</span>
                                        </label>
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="update_logo" value="no" class="form-radio" checked onchange="toggleLogoUpload(false)">
                                ...
                                <input type="file" name="logofile" id="logofile" accept="image/*"
                                    ...
                                    onchange="previewLogo(event)" disabled>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            ...
                            <div>
                                ...
                                <input type="time" name="{{ $day }}_hour_from" id="{{ $day }}_hour_from"
                                    value="{{ old($day.'_hour_from', $store->{$day.'_hour_from'} ? \Carbon\Carbon::parse($store->{$day.'_hour_from'})->format('H:i') : '') }}"
                            ...
                            <div>
                                ...
                                <input type="time" name="{{ $day }}_hour_to" id="{{ $day }}_hour_to"
                                    value="{{ old($day.'_hour_to', $store->{$day.'_hour_to'} ? \Carbon\Carbon::parse($store->{$day.'_hour_to'})->format('H:i') : '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-gray-500 focus:border-gray-500">
                            </div>
                            <div>
                                <label for="{{ $day }}_dayoff" class="flex items-center cursor-pointer">
                                    <input type="checkbox" name="{{ $day }}_dayoff" id="{{ $day }}_dayoff"
                                        value="Yes" {{ old($day.'_dayoff', $store->{$day.'_dayoff'}) == 'Yes' ? 'checked' : '' }}
    ...
    <script>
        ...
        function toggleLogoUpload(enable) {
