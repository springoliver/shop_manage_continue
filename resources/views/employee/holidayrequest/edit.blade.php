...
<x-employee-app-layout>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-6">
            <form action="{{ route('employee.holidayrequest.update', base64_encode($holidayRequest->requestid)) }}" method="POST" id="myform">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" 
                               name="from_date" 
                               id="from_date" 
                               value="{{ old('from_date', \Carbon\Carbon::parse($holidayRequest->from_date)->format('d-m-Y')) }}"
                               autocomplete="off"
                               required
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="to_date" 
                               id="to_date" 
                               value="{{ old('to_date', \Carbon\Carbon::parse($holidayRequest->to_date)->format('d-m-Y')) }}"
                               autocomplete="off"
                               required
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500">
                        @error('to_date')
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="subject" 
                               id="subject" 
                               value="{{ old('subject', $holidayRequest->subject) }}"
                               required
                               maxlength="255"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500">
                        @error('subject')
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  ...
                                  rows="4"
                                  required
                                  class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500">{{ old('description', $holidayRequest->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
